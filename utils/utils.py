...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, multi_label=True, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        boxes_numpy = boxes.detach().cpu().numpy()
        boxes_list = []
        for id_i in range(boxes_numpy.shape[0]):
            boxes_list.append([int(boxes_numpy[id_i, 0]), int(boxes_numpy[id_i, 1]), int(boxes_numpy[id_i, 2]), int(boxes_numpy[id_i, 3])])

        scores_list =  np.float32(scores.detach().cpu().numpy()).tolist()

        ii = cv2.dnn.NMSBoxes(boxes_list, scores_list, conf_thres, iou_thres)
        ii_new = []
        for ii_id in ii:
            ii_new.append(ii_id[0])
        # print(ii_new, len(ii_new))
        # i = torchvision.ops.boxes.nms(boxes, scores, iou_thres)
        # print(i, i.shape)
        # print(torch.IntTensor(ii_new), i)
        i = torch.LongTensor(ii_new)
